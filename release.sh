...
#   5. If current branch is master, create or checkout a train branch.
#   6. Check we're on a train branch.
...
if [ "$CURRENT_BRANCH" = "master" ]; then
  echo 'Warning: `master` branch detected, switching to train branch.'
...
# 6. Check we're on a train branch.
TRAIN_BRANCH=`git branch | grep '^\*' | cut -d ' ' -f 2`
IS_TRAIN_BRANCH=`echo "$TRAIN_BRANCH" | cut -d '-' -f 1`
if [ "$IS_TRAIN_BRANCH" != "train" ]; then
  abort "This is not a train branch"
fi
